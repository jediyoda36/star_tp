#ifndef LIGHT_THREAD_SAFE_UNORDERED_MAP_H_
#define LIGHT_THREAD_SAFE_UNORDERED_MAP_H_
#include "RWLock.h"
#include <memory>
...
class ThreadSafeUnorderedMap {
  ...
  public:
    ...
    explicit ThreadSafeUnorderedMap(size_type __n, const hasher &__hf = hasher(),
    ...
    ThreadSafeUnorderedMap(const map_type &v) : map(v) {}
    ThreadSafeUnorderedMap(map_type &&rv) : map(std::move(rv)) {}
    explicit ThreadSafeUnorderedMap(const allocator_type &__a) : map(__a) {}
    ThreadSafeUnorderedMap(const map_type &__umap, const allocator_type &__a)
        : map(__umap, __a) {}
    ThreadSafeUnorderedMap(map_type &&__umap, const allocator_type &__a)
        : map(std::move(__umap), __a) {}
    ThreadSafeUnorderedMap(initializer_list<value_type> __l, size_type __n = 0,
                             const hasher &__hf = hasher(), const key_equal &__eql = key_equal(),
                             const allocator_type &__a = allocator_type())
    ...
    template <typename _InputIterator>
    ThreadSafeUnorderedMap(_InputIterator __first, _InputIterator __last, size_type __n,
    ...
    ThreadSafeUnorderedMap(initializer_list<value_type> __l, size_type __n,
                             const allocator_type &__a)
        : ThreadSafeUnorderedMap(__l, __n, hasher(), key_equal(), __a) {}
    ThreadSafeUnorderedMap(initializer_list<value_type> __l, size_type __n, const hasher &__hf,
                             const allocator_type &__a)
...
#endif /* LIGHT_THREAD_SAFE_UNORDERED_MAP_H_ */
